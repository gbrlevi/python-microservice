...
async def create_plano_mestre(plano_data: models.PlanoMestreCreate):
    ...
    result = config.planos_mestre_collection.insert_one(plano_dict)
    plano_criado = config.planos_mestre_collection.find_one({"_id": result.inserted_id})

    return plano_criado

@router.get("/", response_model=List[models.PlanoMestre])
async def get_all_planos_mestre():
    planos_cursor = config.planos_mestre_collection.find()
...
async def get_plano_mestre(plano_id: PyObjectId):
    plano_mestre_db = config.planos_mestre_collection.find_one({"_id": plano_id})
    if not plano_mestre_db:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Plano Mestre não encontrado")

    itens_db = config.itens_plano_collection.find({"plano_mestre_id": plano_id})
...
async def update_plano_mestre(plano_id: PyObjectId, plano_data: models.PlanoMestreBase):
    ...
    updated_plano = config.planos_mestre_collection.find_one_and_update(
        {"_id": plano_id},
        {"$set": update_dict},
...
async def delete_plano_mestre(plano_id: PyObjectId):
    config.itens_plano_collection.delete_many({"plano_mestre_id": plano_id})
    result = config.planos_mestre_collection.delete_one({"_id": plano_id})
...
async def create_item_plano(item_data: models.ItemPlanoCreate):
    if not config.planos_mestre_collection.find_one({"_id": item_data.plano_mestre_id}):
    ...
    result = config.itens_plano_collection.insert_one(item_dict)
    item_criado = config.itens_plano_collection.find_one({"_id": result.inserted_id})
    return item_criado

@router.get("/{plano_id}/itens", response_model=List[models.ItemPlano])
async def get_itens_for_plano(plano_id: PyObjectId):
    if not config.planos_mestre_collection.find_one({"_id": plano_id}):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Plano Mestre não encontrado")

    itens_cursor = config.itens_plano_collection.find({"plano_mestre_id": plano_id})
...
async def get_item_plano(item_id: PyObjectId):
    item_db = config.itens_plano_collection.find_one({"_id": item_id})
    if not item_db:
...
async def update_item_plano(item_id: PyObjectId, item_data: models.ItemPlanoBase):
    ...
    updated_item = config.itens_plano_collection.find_one_and_update(
        {"_id": item_id},
        {"$set": update_dict},
        return_document=ReturnDocument.AFTER
    )
...
async def delete_item_plano(item_id: PyObjectId):
    result = config.itens_plano_collection.delete_one({"_id": item_id})
    if result.deleted_count == 0:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Item de Plano com ID '{item_id}' não encontrado.")
    return